podTemplate(yaml: readTrusted('pod.yaml')) {
  node(POD_LABEL) {
    ...
    stage ('build docker image') {
      container ('buildkit') {
        sh """
          ...
          --local dockerfile=.\
          --output type=image,name=134448505602.dkr.ecr.ap-south-1.amazonaws.com/dev/eos-payment-api:latest,push=true
          """
      }
    }
    stage ('package helm chart and push aws ecr repository') {
      container('aws-cli-helm') {
        sh """
          helm package eos-payment-api-chart && ls -l
          helm push eos-payment-api-0.1.0.tgz oci://134448505602.dkr.ecr.ap-south-1.amazonaws.com/dev/helm/
          aws ecr describe-images --repository-name dev/helm/eos-payment-api --region ap-south-1
          """
      }
    }
  }
